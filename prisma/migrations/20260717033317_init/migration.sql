...
-- SCHEMAS
DROP SCHEMA IF EXISTS ubi CASCADE;
DROP SCHEMA IF EXISTS core CASCADE;
DROP SCHEMA IF EXISTS entrenador CASCADE;
DROP SCHEMA IF EXISTS alumno CASCADE;
...
CREATE TYPE app_user.user_role AS ENUM ('alumno', 'entrenador', 'admin','alumno_con_entrenador');

-- TABLAS CORE
CREATE TABLE core.objetivo (
...
CREATE TABLE core.nivel_actividad (
  id SMALLINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  codigo VARCHAR(50) NOT NULL UNIQUE,
  nombre VARCHAR(100) NOT NULL,
  descripcion TEXT,
...
CREATE TYPE app_user.estado_suscripcion_enum AS ENUM (
  'prueba',
  'activa',
  'morosa',
  'cancelada',
...
CREATE TABLE alumno.alumno (
  ...
  id_objetivo SMALLINT,
  id_nivel_actividad SMALLINT,
...
CREATE TABLE alumno.token_vinculacion_alumno (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY NOT NULL,
  id_alumno UUID NOT NULL,
  codigo_hash CHAR(64) NOT NULL,
  secreto_hash CHAR(64) NOT NULL,
  creado_en TIMESTAMP(0) DEFAULT NOW(),
  expira_en TIMESTAMP(0),
  usado_en TIMESTAMP(0),
  revocado_en TIMESTAMP(0),
  reclamado_por_entrenador UUID,
  actualizado_en TIMESTAMP(0) DEFAULT NOW(),
  actualizado_por UUID,
  CONSTRAINT token_vinculacion_alumno_expira_check
...
CREATE INDEX token_vinculacion_alumno_id_alumno_idx
  ON alumno.token_vinculacion_alumno (id_alumno);

CREATE INDEX token_vinculacion_alumno_codigo_hash_idx
  ON alumno.token_vinculacion_alumno (codigo_hash);

CREATE INDEX token_vinculacion_alumno_secreto_hash_idx
  ON alumno.token_vinculacion_alumno (secreto_hash);

CREATE UNIQUE INDEX token_vinculacion_alumno_unico_activo_idx
  ON alumno.token_vinculacion_alumno (id_alumno)
  WHERE usado_en IS NULL AND revocado_en IS NULL;

-- FOREIGN KEYS
ALTER TABLE alumno.token_vinculacion_alumno
  ADD CONSTRAINT token_vinculacion_alumno_id_alumno_foreign
  FOREIGN KEY (id_alumno) REFERENCES alumno.alumno (id_usuario)
  DEFERRABLE INITIALLY IMMEDIATE;

ALTER TABLE alumno.token_vinculacion_alumno
  ADD CONSTRAINT token_vinculacion_alumno_reclamado_por_entrenador_foreign
  FOREIGN KEY (reclamado_por_entrenador) REFERENCES entrenador.entrenador (id_usuario)
  DEFERRABLE INITIALLY IMMEDIATE;
  
ALTER TABLE alumno.token_vinculacion_alumno
  ADD CONSTRAINT token_vinculacion_alumno_actualizado_por_foreign
  FOREIGN KEY (actualizado_por) REFERENCES app_user.usuario (id)
  DEFERRABLE INITIALLY IMMEDIATE;
...
ALTER TABLE app_user.usuario
  ADD CONSTRAINT usuario_id_sexo_foreign
  FOREIGN KEY (id_sexo) REFERENCES core.sexo (id) DEFERRABLE INITIALLY IMMEDIATE;
...
ALTER TABLE entrenador.entrenador
  ADD CONSTRAINT entrenador_id_plan_foreign
  FOREIGN KEY (id_plan) REFERENCES app_user.plan_entrenador (id) DEFERRABLE INITIALLY IMMEDIATE;

ALTER TABLE alumno.alumno
  ADD CONSTRAINT alumno_id_plan_foreign
  FOREIGN KEY (id_plan) REFERENCES app_user.plan_alumno (id) DEFERRABLE INITIALLY IMMEDIATE;
...
ALTER TABLE alumno.alumno
  ADD CONSTRAINT alumno_id_objetivo_foreign
  FOREIGN KEY (id_objetivo)
  REFERENCES core.objetivo(id);

ALTER TABLE alumno.alumno
  ADD CONSTRAINT alumno_id_nivel_actividad_foreign
  FOREIGN KEY (id_nivel_actividad)
  REFERENCES core.nivel_actividad(id);
...
ALTER TABLE app_user.suscripcion
  ADD CONSTRAINT suscripcion_alumno_foreign
    ...
    REFERENCES alumno.alumno(id_usuario);

ALTER TABLE app_user.suscripcion
  ADD CONSTRAINT suscripcion_entrenador_foreign
